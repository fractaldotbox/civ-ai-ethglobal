...
export const generateEmptyGrid =  (rowSize:number, columnSize:number)=> Array(columnSize).fill(null).map(() => Array(rowSize).fill(null).map(()=>({
 ...
 * 
 */
export const genearteGridWithNonOverlappingTiles = (rowSize:number, columnSize:number, inputTiles:Partial<Tile>[])=>{
    
    const nonOverlappingTiles = [...inputTiles]

    const fillerTiles = Array(rowSize * columnSize - nonOverlappingTiles.length).fill(null);
...
export const genearteGridWithOverlappingTiles = (rowSize:number, columnSize:number,  tileByType:GameSeed['tileByType'], tileResourceMax:number)=>{

    const resourceTiles = _.flatten(Object.entries(tileByType).map(([resource, {total}])=>{
        return Array(total).fill({resource})
    }))

    
    const fillerTiles = Array(rowSize * columnSize * tileResourceMax - resourceTiles.length).fill(null);
    ...
    for(let i = 0; i < rowSize; i++) {
        for(let j = 0; j < columnSize; j++) {
            const resourceOfTiles  = resources.slice((i+j) * tileResourceMax, (i+j+1) * tileResourceMax);

            resourceOfTiles.forEach((resourceOfTile?:{resource:TileResource})=>{
                const {resource} = resourceOfTile || {};
                if(resource){
                    grid[i]![j]!.resourceByType[resource] = (grid[i]![j]!.resourceByType[resource] || 0) + 1;
                }
...
export const generateRandomGrid = (gameSeed:GameSeed)=>{
    const {rowSize, columnSize, playerCount, tileByType, tileResourceMax } = gameSeed;
    ...
    const playerGrid = genearteGridWithNonOverlappingTiles(rowSize, columnSize, nonOverlappingTiles);

    const resourceGrid = genearteGridWithOverlappingTiles(rowSize, columnSize, tileByType, tileResourceMax)
